# Plan Scope — guida per chi modifica il codice

Chi cerca cos'è l'app, come si prova e a quali condizioni se ne usa il codice legge il
[README](README.md). Questo file è per chi apre questa cartella per cambiarla, persona o modello che sia.
...
`biz/plan-model.js` sta in `app/_business/` perché lo usano due app: **Plan Scope e Invoice Scope**. Un
progetto esportato da una si apre nell'altra.
...
| `biz/ics.js` · `csv.js` · `webpage.js` | calendario, foglio di calcolo, pagina HTML autosufficiente | puri: stringhe dentro, stringhe fuori |
...
Passano dalla import map, mai da un import relativo, e hanno due licenze diverse:

- **`gg/` → `app/_lib/`**, Apache-2.0, comune a tutte le app: `store.js`, `io.js`, `dom.js`,
  `i18n.js`, `theme.js`, `install.js`, `update.js`;
- **`biz/` → `app/_business/`**, PolyForm Shield 1.0.0 come l'app, comune alle app gestionali:
  `plan-model.js`, `plan-markdown.js`, `plan-editor.js`, `plan-pack.js`, `zip.js`, `folder.js`,
  `ics.js`, `remind.js`.

Un modulo scritto per le sole app gestionali va in `_business/`; spostarlo in `_lib/` lo renderebbe
Apache-2.0, e non si torna indietro.

> **Toccare `_business/plan-*.js` tocca anche Invoice Scope.** Sono lo stesso file, e ogni service
> worker se lo mette nella propria cache. Una modifica lì obbliga a girare la versione di **entrambe**
> le app: `check_apps.py` non se ne accorge, è una regola umana (`app/CLAUDE.md`).

---

## Le otto cose che non si rompono

1. **`biz/plan-model.js` non conosce il browser, `db.js` non conosce i progetti.** Se ci si trova a importare
...
stanno in `_business/`, quindi ogni prova passa dal loader, che risolve `gg/` e `biz/`.
...
node $P app/plan-scope/test/tip.mjs         # il fumetto dei segni: mouse, tastiera, tocco
node $P app/plan-scope/test/home.mjs        # home e pagina del progetto, sul markup vero
```

Cinque pezzi di impalcatura, che non sono prove e non si lanciano: `test/fake-db.mjs`,
`test/fake-folder.mjs`, `test/fake-archive.mjs`, `test/dom.mjs` (gli alberi statici dell'editor) e
`test/fake-page.mjs`, una pagina i cui eventi risalgono l'albero come in un browser e i cui id
vengono da `run/index.html`. Una schermata si prova lì sopra, non aprendo la pagina.
...
- [ ] `ASSETS` in `sw.js` elenca esattamente quello che l'app importa, `_lib/` e `_business/` compresi
...
- [ ] Se si è toccato `_business/`: girata anche la versione di **Invoice Scope**
