...
### Cosa è coperto da una licenza, e da quale

Solo le app in [`app/`](app/), ognuna con `LICENSE` e `NOTICE` nella propria cartella.

| Cartella | Licenza |
|---|---|
| `app/plan-scope/`, `app/invoice-scope/`, `app/survey-scope/`, `app/_business/` — le app gestionali e la loro libreria | [PolyForm Shield 1.0.0](https://polyformproject.org/licenses/shield/1.0.0/) |
| I questionari di Survey Scope, `app/survey-scope/run/*/*.json` | Apache-2.0 |
| Le altre app e la libreria comune, `app/_lib/` | Apache-2.0 |

Con la PolyForm Shield il codice è pubblico: si legge, si modifica e si usa anche in un lavoro
commerciale. Ogni copia porta con sé la licenza e le righe `Required Notice:` del `NOTICE`, che
nominano G&G Technologies come autore. Un prodotto che fa lo stesso lavoro, venduto o gratuito,
esteso o con un proprio backend, richiede una licenza commerciale: quando serve e a quali
condizioni lo dice `COMMERCIAL-LICENSE.md`, accanto a ogni `LICENSE`, e si chiede a
[info@ggtechnologies.sm](mailto:info@ggtechnologies.sm). Le versioni pubblicate prima del
25 settembre 2026 restano Apache-2.0.

Tutto il resto — le pagine del sito, gli articoli, il marchio — non ha licenza: tutti i diritti
riservati.
...
Licences cover the apps in `app/` only, each with its own `LICENSE` and `NOTICE`. The business apps
(Plan Scope, Invoice Scope, Survey Scope) and their library `app/_business/` are under PolyForm
Shield 1.0.0: public source, free for your own work, a commercial licence for a competing product
(terms in `COMMERCIAL-LICENSE.md`, next to each `LICENSE`).
Survey Scope's questionnaires, the other apps and `app/_lib/` are under Apache-2.0. The pages, the
articles and the brand are all rights reserved.
